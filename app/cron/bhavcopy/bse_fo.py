"""
BSE F&O bhavcopy — download or register.

Saved as : <DATA_PATH>/bhavcopy/<YYYY-MM-DD>/BhavCopy_BSE_FO_0_0_0_YYYYMMDD_F_0000.CSV
...
CLI:
  python -m app.cron.bhavcopy.bse_fo --date 2026-01-01
  python -m app.cron.bhavcopy.bse_fo --file /app/inbox/BhavCopy_BSE_FO_0_0_0_20260101_F_0000.CSV
"""
...
SOURCE     = "BSE_FO"
_FNAME_TPL = "BhavCopy_BSE_FO_0_0_0_{}_F_0000.CSV"
_URL_TPL   = ("https://www.bseindia.com/download/BhavCopy/Derivative/"
              "BhavCopy_BSE_FO_0_0_0_{}_F_0000.CSV")
_INBOX_RE  = re.compile(
    r"BhavCopy_BSE_FO_0_0_0_(\d{8})_F_0000\.csv$", re.IGNORECASE
)
...
def register(file_path: Path, force: bool = False) -> bool:
    ...
    if not match:
        logger.error("[%s] Filename does not match BSE FO pattern: %s", SOURCE, file_path.name)
        return False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="BSE F&O bhavcopy downloader/register")
